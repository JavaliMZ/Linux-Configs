...
setopt NO_HIST_VERIFY      # Do subsitution off expansions (!!,  !?, $(>file.ext)...) without confirmation
...
# 		twoline)
# 			PROMPT=$'%F{%(#.blue.green)}┌──${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n$prompt_symbol%m%b%F{%(#.blue.green)})-[%B%F{reset}%(4~|…/%2~|%~)%b%F{%(#.blue.green)}]\n└─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
# 			RPROMPT=$'%(?.. %? %F{red}%B⨯%b%F{reset})%(1j. %j %F{yellow}%B⚙%b%F{reset}.)'
# 			;;
# 		oneline)
# 			PROMPT=$'%F{%(#.blue.green)}${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}%n$prompt_symbol%m%b%F{%(#.blue.green)})-[%B%F{reset}$(shrink_path -f)%b%F{%(#.blue.green)}]─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
# 			RPROMPT=
# 			;;
# 		backtrack)
...
configure_prompt() {
	...
	case "$PROMPT_ALTERNATIVE" in
		twoline)
			...
			;;
		oneline)
			PROMPT=$'%F{%(#.blue.green)}${debian_chroot:+($debian_chroot)─}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))─}(%B%F{%(#.red.blue)}JavaliMZ${prompt_symbol}kali%b%F{%(#.blue.green)})-[%B%F{reset}$(shrink_path -f)%b%F{%(#.blue.green)}]─%B%(#.%F{red}#.%F{blue}$)%b%F{reset} '
			RPROMPT=
			;;
		backtrack)
			PROMPT=$'${debian_chroot:+($debian_chroot)}${VIRTUAL_ENV:+($(basename $VIRTUAL_ENV))}%B%F{red}%n@%m%b%F{reset}:%B%F{blue}%~%b%F{reset}%(#.#.$) '
...
alias ll='lsd -lArth --color=auto --group-dirs first'
alias dir='dir --color=auto'
alias vdir='vdir --color=auto'
alias grep='grep --color=auto'
alias fgrep='fgrep --color=auto'
...
alias ip="ip -color -family inet -human -oneline -brief add | sed 's/\/.*//g'"
alias edit_bspwmrc='code /home/javali/.config/bspwm/bspwmrc'
alias edit_sxhkd='code /home/javali/.config/sxhkd/sxhkdrc'
alias edit_compton='code /home/javali/.config/compton/compton.conf'
alias edit_zshrc='code /home/javali/.zshrc'
alias ..='cd ..'
alias ...='cd ../..'
alias ....='cd ../../..'
alias .....='cd ../../../..'
alias ......='cd ../../../../..'
alias disableLowPortsProtection='sudo /usr/sbin/sysctl --system'
alias -g into_clip='| clip.exe'
alias pwncat='pwncat-cs'

# Path
export PATH=$PATH:.:/home/javali/.local/bin:/home/javali/.cargo/bin

# Function
extractUrl() {
	cat $1 |jq . | grep -E '"url": "[^"]*",' | tr -d '",' | awk '{print$2}'
}
...
newctf(){
	mkdir enumeration exploits contents
...
extractPorts() {
	...
	cat /tmp/nmapTmp.txt; rm /tmp/nmapTmp.txt
...
getSttyDimentions() {
	echo -n "stty "
	stty -a | grep "columns" | sed 's/;/ /g' | awk '{print $4, $5, $6, $7}'
}


HTBConnection() {
	sudo nohup openvpn /home/javali/Vpn/Javali.ovpn &>/dev/null &
	disown &>/dev/null
}
...
# enable sudo.plugin.zsh if installed
if [ -f /usr/share/zsh-sudo/sudo.plugin.zsh ]; then
	. /usr/share/zsh-sudo/sudo.plugin.zsh
fi


export IP=$(/home/javali/.local/bin/getTarget.sh | awk '{print $1}')
